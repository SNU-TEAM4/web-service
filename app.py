...
def render_kakao_map(lat: float, lon: float, radius_km: int, stores: pd.DataFrame) -> None:
    ...
    </head><body><div id="map"></div><div id="status">카카오맵을 불러오는 중입니다…</div><script>
    const statusBox=document.getElementById('status');
    function mapError(message){{statusBox.style.display='flex';statusBox.innerHTML='카카오맵을 표시하지 못했습니다.<br>'+message+'<br>페이지를 새로고침한 뒤 다시 검색해 주세요.';}}
    ...
    if(typeof kakao==='undefined'||!kakao.maps){{
      mapError('카카오 지도 SDK를 초기화하지 못했습니다.');
    }} else {{
      const renderer=document.createElement('script');
      renderer.src='https://t1.daumcdn.net/mapjsapi/js/main/4.5.26/kakao.js';
      renderer.onload=function(){{
        if(typeof kakao.maps.LatLng==='function'&&typeof kakao.maps.Map==='function') initMap();
        else mapError('카카오 지도 클래스를 준비하지 못했습니다.');
      }};
      renderer.onerror=function(){{mapError('카카오 지도 렌더링 모듈 다운로드가 차단됐습니다.')}};
