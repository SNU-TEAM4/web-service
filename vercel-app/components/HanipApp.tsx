  ...
  return (
    <main>
      ...
      <aside className={`quick-filter-panel ${showQuickFilters && !quickFiltersMinimized ? "visible" : ""} ${quickFiltersOpen ? "open" : ""}`}>
        ...
        <div className="quick-group"><h3>안전 상태</h3><div className="quick-safety"><button className={safetyMode === "safe" ? "active safe" : ""} onClick={() => setSafetyMode("safe")}>안전</button><button className={safetyMode === "danger" ? "active danger" : ""} onClick={() => setSafetyMode("danger")}>위험</button><button className={safetyMode === "all" ? "active" : ""} onClick={() => setSafetyMode("all")}>모두</button></div></div>
...
function DetailComparePanel({ menus, selection, setSelection, cartIds }: { menus: Menu[]; selection: number[]; setSelection: React.Dispatch<React.SetStateAction<number[]>>; cartIds: number[] }) {
  ...
  return <section className="panel detail-compare"><div className="panel-head"><div><span className="compare-kicker">NUTRITION COMPARE</span><h2>메뉴 영양성분 비교</h2><p>최대 4개 메뉴의 영양 균형을 같은 기준으로 비교해요.</p></div></div>
    ...
    <div className="compare-step"><span>01</span><b>직접 메뉴 추가</b><small>{selection.length}/4 선택</small></div>
    ...
    {!selected.length ? <div className="empty"><h3>비교할 메뉴를 선택해 주세요</h3><p>직접 검색하거나 ‘나의 한 끼’에 담은 메뉴를 한 번에 불러올 수 있어요.</p></div> : <>
