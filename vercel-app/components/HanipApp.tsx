...
const NUTRITION_PRESETS: Array<{
  ...
  { id: "default", title: "기본 균형", detail: "600kcal · 나트륨 1,500mg", maxCalories: 600, minProtein: 0, maxSodium: 1500 },
  { id: "light", title: "가벼운 한 끼", detail: "450kcal · 단백질 10g", maxCalories: 450, minProtein: 10, maxSodium: 1000 },
  { id: "protein", title: "고단백 탐색", detail: "단백질 20g · 700kcal", maxCalories: 700, minProtein: 20, maxSodium: 1800 },
  { id: "sodium", title: "저나트륨 우선", detail: "나트륨 600mg 이하", maxCalories: 700, minProtein: 0, maxSodium: 600 },
];
  ...
  return (
    <main>
      ...
      <aside className={`quick-filter-panel ${showQuickFilters && !quickFiltersMinimized ? "visible" : ""} ${quickFiltersOpen ? "open" : ""}`}>
        ...
        <div className="quick-group"><h3>안전 상태</h3><div className="quick-safety"><button className={safetyMode === "all" ? "active" : ""} onClick={() => setSafetyMode("all")}>모두</button><button disabled={!allergens.length} className={safetyMode === "safe" ? "active safe" : ""} onClick={() => setSafetyMode("safe")}>안전</button><button disabled={!allergens.length} className={safetyMode === "danger" ? "active danger" : ""} onClick={() => setSafetyMode("danger")}>위험</button></div></div>
      ...
      <section className="quick-start-shell" id="quick-start">
        <Reveal><section className="quick-start">
          <div className="preset-copy">
            <span>내 기준에 맞춰 보기</span>
            <h2>먹고 싶은 메뉴를,<br />내 조건에 맞게.</h2>
            <p>빠른 조건은 영양 범위만 바꿉니다. 선택한 브랜드와 확인할 알레르기 성분은 그대로 유지됩니다.</p>
            <div className="preset-grid">
              {NUTRITION_PRESETS.map((preset) => <button key={preset.id} className={nutritionPreset === preset.id ? "active" : ""} onClick={() => applyNutritionPreset(preset.id)}><b>{preset.title}</b><small>{preset.detail}</small></button>)}
            </div>
          </div>
            ...
            {spotlightMenu ? <>
              <div className="spotlight-copy">
                ...
                <dl className="spotlight-nutrition">
                  ...
                  <div><dt>{spotlightMenu.protein.toFixed(0)}</dt><dd>단백질 g</dd></div>
                  <div><dt>{spotlightMenu.sodium.toFixed(0)}</dt><dd>나트륨 mg</dd></div>
                </dl>
                <div className="spotlight-price"><b>{spotlightMenu.price ? `${spotlightMenu.price.toLocaleString()}원` : "가격 확인 중"}</b><small>{spotlightMenu.price ? spotlightMenu.priceNote : "매장·주문 채널에 따라 달라질 수 있어요"}{spotlightMenu.priceCheckedAt ? ` · ${spotlightMenu.priceCheckedAt}` : ""}</small></div>
              </div>
              <div className="spotlight-media">
                <div className="spotlight-brand-logo"><BrandLogo brand={spotlightMenu.brand} size={54} /></div>
                <img src={spotlightMenu.imageUrl || brandFallbackImage(spotlightMenu.brand)} alt={`${spotlightMenu.menu} 메뉴 이미지`} onError={(event) => { event.currentTarget.src = brandFallbackImage(spotlightMenu.brand); }} />
              </div>
              <div className="spotlight-actions">
                <button onClick={() => setSpotlightIndex((current) => current + 1)}><RefreshCw size={17} /> 다른 메뉴</button>
                <button className="primary" onClick={(event) => addToCart(spotlightMenu.id, event)}><UtensilsCrossed size={17} /> 한 끼에 담기</button>
                {spotlightOfficialUrl ? <a href={spotlightOfficialUrl} target="_blank" rel="noreferrer"><ExternalLink size={17} /> 공식 정보</a> : <button onClick={() => changeTab("about")}><ExternalLink size={17} /> 데이터 기준</button>}
              </div>
            </> : <div className="spotlight-empty"><UtensilsCrossed size={34} /><h3>조건에 맞는 한 끼를 찾고 있어요</h3><p>브랜드나 영양 조건을 조금 넓히면 추천 메뉴를 보여드릴게요.</p></div>}
          </article>
        </section></Reveal>
      </section>
      ...
      <section className="content" id="explorer">
        ...
        <div ref={filtersAnchorRef}><Reveal><section className="horizontal-filters">
          ...
          <div className="filter-block"><h3>메뉴 안전 상태</h3><p>{allergens.length ? "선택한 알레르기 기준" : "표시 성분 유무 기준"}</p><div className="safety-options">
            ...
            <button disabled={!allergens.length} className={`safe-option ${safetyMode === "safe" ? "active" : ""}`} onClick={() => setSafetyMode("safe")}>안전한 것만</button>
            <button disabled={!allergens.length} className={`danger-option ${safetyMode === "danger" ? "active" : ""}`} onClick={() => setSafetyMode("danger")}>위험한 것만</button>
          </div></div>
        ...
        {tab === "menus" && <Reveal><section className="panel">
          ...
          <div className="brand-browser"><div className="brand-folders">{grouped.map(([brand, items]) => { const safeCount = items.filter((menu) => !menu.catalogOnly && menu.allergenKnown && allergens.every((item) => !menu.allergens.includes(item))).length; const dangerCount = allergens.length ? items.filter((menu) => menu.allergenKnown && allergens.some((item) => menu.allergens.includes(item))).length : 0; return <button className={`brand-tile ${activeBrand === brand ? "selected" : ""} ${items.length ? "" : "data-pending"}`} key={brand} onClick={() => { setOpenBrands(activeBrand === brand ? {} : { [brand]: true }); setMenuSectionFilter("전체"); }}>
            <BrandLogo brand={brand} size={78} /><span><b>{brand}</b>{items.length > 0 && <small><em className="safe-count">안전 {safeCount}개</em>{dangerCount > 0 && <em className="danger-count">위험 {dangerCount}개</em>}</small>}</span>{activeBrand === brand ? <Check /> : <ChevronRight />}
          </button>; })}</div>
