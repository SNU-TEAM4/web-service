...
function Range({ label, value, min, max, step, unit, onChange }: { label: string; value: number; min: number; max: number; step: number; unit: string; onChange: (value: number) => void }) { const protein = /단백질|Protein/.test(label); const unlimited = protein ? value === min : value === max; const unlimitedLabel = /[A-Za-z]/.test(label) ? "No limit" : "제한 없음"; return <label className="range"><span>{label}<b>{unlimited ? unlimitedLabel : `${value} ${unit}`}</b></span><input type="range" value={value} min={min} max={max} step={step} onChange={(e) => onChange(Number(e.target.value))} /></label>; }
