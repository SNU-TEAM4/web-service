...
export default function HanipApp() {
  ...
  const filtered = useMemo(() => menus.filter((menu) => {
    ...
    const nutritionMatch = menu.catalogOnly || (
      (!menu.caloriesKnown || (menu.calories >= minCalories && menu.calories <= maxCalories))
  ...
  }).sort(compareMenus), [menus, allergens, brands, brandCategory, safetyMode, minCalories, maxCalories, minProtein, maxFat, maxCarbs, maxSodium, query, profileOn, targetCalories, sortMode]);
...
function RangeSlider({ label, minValue, maxValue, min, max, step, unit, onChange }: { label: string; minValue: number; maxValue: number; min: number; max: number; step: number; unit: string; onChange: (min: number, max: number) => void }) {
  const start = ((minValue - min) / (max - min)) * 100;
  const end = ((maxValue - min) / (max - min)) * 100;
  return <div className="range range-double"><span>{label}<b>{minValue}–{maxValue} {unit}</b></span><div className="range-double-control" style={{ "--range-start": `${start}%`, "--range-end": `${end}%` } as React.CSSProperties}><input aria-label={`${label} minimum`} type="range" value={minValue} min={min} max={max} step={step} onChange={(e) => onChange(Math.min(Number(e.target.value), maxValue), maxValue)} /><input aria-label={`${label} maximum`} type="range" value={maxValue} min={min} max={max} step={step} onChange={(e) => onChange(minValue, Math.max(Number(e.target.value), minValue))} /></div></div>;
}
